  ...
  <div class="register-container">
    <div class="register-box">
      ...
      <form @submit.prevent="register">
        ...
        <div class="input-group">
          <input v-model="price" type="text" placeholder="借出时间" required>
        </div>
        <div class="input-group">
...
const register = async () => {
  try {
    ...
    formData.append('book_id', ISBN.value); // ISBN 对应 book_id
    formData.append('user_id', adminid.value); // 管理员 ID 对应 user_id
    formData.append('category', category.value);
    formData.append('bookname', bookname.value);

    // 发送 POST 请求到 /lend/addRecord 接口
    const response = await axios.post('http://localhost:8080/lend/addRecord', formData, {
      headers: {
