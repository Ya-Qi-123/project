<template>
  <div>
    ...
    >
      <el-table-column prop="id" label="收藏编号" width="120" />
      <el-table-column prop="bookName" label="书名" width="140" />
      <el-table-column prop="isbn" label="isbn" width="150" />
      <el-table-column prop="userId" label="收藏用户（我）" width="190" />
      <el-table-column prop="author" label="书籍作者" width="180" />
    ...
    <el-dialog
        ...
        width="30%"
    >
      <span>确定要删除收藏书籍 {{ currentDeleteRow?.bookName }} 吗？</span>
      <template #footer>
...
const fetchBooksByPage = async () => {
  ...
  try {
    const response = await service.get('/api/favorite/pageQuery', {
      params: {
...
const confirmDelete = async () => {
  ...
  try {
    // 调用后端接口删除数据
    const response = await axios.delete('/api/favorite/removeById', {
      params: {
